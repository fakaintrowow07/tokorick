...
class LaporanAdapter(val listTransaksi :  ArrayList<Transaksi>):RecyclerView.Adapter<LaporanAdapter.ViewHolder>() {
    ...
    fun sortTotal(asc: Boolean) {
        if (asc) {
            listTransaksi.sortBy { produk -> produk.total.toInt() }
        } else {
            listTransaksi.sortByDescending { produk -> produk.total.toInt() }
        }
